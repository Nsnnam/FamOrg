...
Khi khởi động lần đầu (hoặc sau khi xóa `data/db.sqlite`), hệ thống tự tạo 4 tài khoản mẫu:

| Vai trò | Tên đăng nhập | Mật khẩu | Quyền |
| :--- | :--- | :--- | :--- |
| **Admin** | `admin` | `admin123` | Toàn quyền |
| **Member** | `bohung` | `bohung123` | Thành viên đầy đủ |
| **Member** | `melan` | `melan123` | Thành viên đầy đủ |
| **Guest** | `bevy` | `bevy123` | Chỉ đọc + nhiệm vụ cá nhân |

> Đổi mật khẩu các tài khoản này ngay sau khi deploy production.
...
```text
./data/
├── db.sqlite       # Database chính (SQLite)
├── backups/        # Backup tự động và thủ công
└── images/         # Ảnh hóa đơn, avatar, tài sản (lưu file, không base64)
```
